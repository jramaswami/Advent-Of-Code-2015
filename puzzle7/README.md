...
logic gates][bitwise-gates]! Unfortunately, little Bobby is a little
under the recommended age range, and he needs help assembling the
circuit.
...
together: `x AND y -> z` means to connect wires `x` and `y`
to an AND gate, and then connect its output to wire `z`.

For example:

* `123 -> x` means that the signal `123` is provided to wire
  `x`.
* `x AND y -> z` means that the [bitwise AND][bitwise-and] of wire
  `x` and wire `y` is provided to wire `z`.  
* `p LSHIFT 2 -> q` means that the value from wire `p` is
  [left-shifted][left-shifted]by `2` and then provided to wire
  `q`.
* `NOT e -> f` means that the [bitwise complement][bitwise-not] of
  the value from wire `e` is provided to wire `f`.

Other possible gates include `OR` ([bitwise OR][bitwise-or]) and
`RSHIFT` ([right-shift][right-shift]). If, for some reason, you'd
like to emulate the circuit instead, almost all programming languages
(for example,
[C][c-bitwise], [JavaScript][javascript-bitwise], or
[Python][python-bitwise]) provide operators for these gates.
...
[c-bitwise]: https://en.wikipedia.org/wiki/Bitwise_operations_in_C
[javascript-bitwise]: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Bitwise_Operators
[python-bitwise]: https://wiki.python.org/moin/BitwiseOperators
