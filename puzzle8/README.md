...
characters in strings. For example, [C][c-escape],
[JavaScript][javascript-escape], [Perl][perl-escape],
[Python][python-escape], and even [PHP][php-escape] handle special
...
* `"aaa\"aaa"` is `10` characters of code, but the string itself
  contains six "a" characters and a single, escaped quote character, for
  a total of `7` characters in` the string data.
...
represents a single character with that ASCII
code).
...
[python-escape]: https://docs.python.org/2.0/ref/strings.html
[php-escape]: http://php.net/manual/en/language.types.string.php#language.types.string.syntax.double
